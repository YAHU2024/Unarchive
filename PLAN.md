...
## 当前实现评审后的修复计划（2026-07-31）

目标：先修复会造成重复数据、配置丢失或视频静默跳过的问题，再补齐同步状态和自动化验证。保持现有 Bilibili/抖音 -> 转写 -> LLM -> 知识卡片 -> 飞书/ima 的产品范围，不扩展新的平台或云端能力。

### P1：必须优先修复

#### 1. 修复 ima 笔记标题与幂等查重

- 调整 `src/sync/ima.py` 的 Markdown 标题构建规则，确保同步传入的 `[video_id] 标题` 能写入笔记标题。
- 保持 `check_document_exists()` 的 `[video_id]` 前缀匹配与实际创建格式一致。
- 覆盖以下场景：首次创建、重复同步、卡片标题为空、卡片标题包含 Markdown 特殊字符。
- 验收：相同视频连续同步两次，第二次不再调用 `import_doc`。

#### 2. 修复 GUI 保存配置覆盖 `.env`

- `do_save_config()` 更新 `.env` 时保留未由当前表单编辑的字段。
- 不丢失 `LLM_PROVIDER`、Whisper、数据目录、CDP 和下载目录等配置。
- 对换行、等号和空值进行安全序列化，避免生成无法解析的 `.env`。
- 验收：保存表单后重新加载配置，所有原有字段值保持不变；敏感值不写入日志。

#### 3. 区分 B站永久不可用与临时错误

- 将 `check_video_available()` 的结果区分为“可用”“明确不可用”“未知/临时失败”，或使用等价的异常类型/结果对象。
- 仅对明确的删除、下架、私密错误码返回不可用。
- 网络错误、风控、HTTP 异常和 Cookie 失效进入重试或失败队列，不显示为已删除并永久跳过。
- 验收：模拟网络异常时视频不会被计入 `unavailable`；明确不可用错误仍会被跳过。

### P2：P1 完成后处理

#### 4. 统一 GUI 与 CLI 的数据目录配置

- 移除 `app.py` 对 `data/knowledge_base` 的硬编码，统一使用 `AppConfig.knowledge_base_dir`。
- 初始化时调用配置目录创建逻辑，并确保 GUI/CLI 读取同一目录。
- 验收：修改 `KNOWLEDGE_BASE_DIR` 后，GUI 和 CLI 能互相看到同一批知识卡片。

#### 5. 明确 ima“建笔记”和“加入知识库”的部分成功状态

- `create_document()` 返回或抛出可区分的结果，不能把知识库关联失败计入完整同步成功。
- UI/CLI 分别显示笔记创建成功、知识库关联成功、关联失败的数量。
- 配额耗尽仍立即停止后续请求，并保留已完成项目的状态。
- 验收：模拟 `add_knowledge` 失败时，日志明确提示部分成功，最终统计不误报为完整成功。

#### 6. 补充最小自动化测试

- 增加 `pytest` 测试，不依赖真实平台、LLM、飞书或 ima 网络请求。
- 至少覆盖：
  - ima 标题构建与重复同步查重；
  - ima 限流重试和配额停止；
  - `.env` 保存时保留未知配置；
  - B站明确不可用与临时错误的分类；
  - GUI/CLI 使用同一知识库目录。
- 为 HTTP API 使用 mock transport，避免消耗真实配额和泄露凭证。

### 实施顺序

1. 先写 P1 行为测试，固定当前问题的复现方式。
2. 修复 ima 标题/查重和配置保存。
3. 修复 B站错误分类及处理管道统计。
4. 统一数据目录并修正 ima 部分成功状态。
5. 运行单元测试、`compileall`、CLI 帮助和 Gradio 启动烟测。
6. 使用真实账号做一次单视频端到端验收；明确记录网络/API/设备相关的未覆盖项。

### 完成标准

- `pytest` 全部通过，且不再出现 `no tests ran`。
- `python -m compileall -q .` 通过。
- `python cli.py --help` 通过。
- Gradio UI 能构建、启动并正常关闭。
- 同一视频重复同步不产生重复 ima 笔记。
- 保存配置不会删除其他环境变量配置。
- 临时平台故障不会被误报为视频已删除。
- 真实 API/账号验收结果与本地自动化结果分开记录。
