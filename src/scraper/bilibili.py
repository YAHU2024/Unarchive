...
class BilibiliScraper(ScraperBase):
    ...
    async def download_video(self, video_id: str, output_path: str) -> bool:
        """下载 Bilibili 视频文件（DASH 格式，音视频合并）

        获取 DASH 播放列表，选择最佳质量的视频流和音频流，
        使用 ffmpeg 合并为完整的 mp4 文件。

        Args:
            video_id: Bilibili BV 号
        ...
        try:
            ...
            if not video_list or not audio_list:
                logger.warning("视频 %s 未找到 DASH 流", video_id)
                return False

            # 选择最佳质量：视频按带宽降序，音频按带宽降序
            best_video = max(video_list, key=lambda v: v.get("bandwidth", 0))
            best_audio = max(audio_list, key=lambda a: a.get("bandwidth", 0))
            video_url = best_video.get("baseUrl") or best_video.get("base_url")
            audio_url = best_audio.get("baseUrl") or best_audio.get("base_url")

            if not video_url or not audio_url:
                logger.warning("视频 %s DASH URL 为空", video_id)
                return False
            ...
            try:
                # 下载视频流
                logger.info("下载视频 %s 视频流...", video_id)
                resp = await self._context.request.get(video_url, headers=_HEADERS)
                if resp.status != 200:
                    return False
                ...
                # 下载音频流
                logger.info("下载视频 %s 音频流...", video_id)
                resp = await self._context.request.get(audio_url, headers=_HEADERS)
                if resp.status != 200:
                    return False
                ...
                # ffmpeg 合并
                logger.info("合并视频 %s 音视频流...", video_id)
                ...
                if result.returncode != 0:
                    logger.warning("ffmpeg 合并失败: %s", result.stderr[:200])
                    return False

                logger.info("视频下载成功: %s", output_path)
                return True
            finally:
                import shutil
            ...
            logger.warning("Bilibili 视频下载失败: %s", e)
            return False
